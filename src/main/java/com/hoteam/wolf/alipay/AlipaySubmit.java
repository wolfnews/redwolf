package com.hoteam.wolf.alipay;
...
import com.hoteam.wolf.utils.HttpClientUtil;
...
public class AlipaySubmit {
	...
	private static final int TIME_OUT = 60 * 100;
	 ...
	 * @throws Exception
	 */
	public static String buildRequest(String strParaFileName, String strFilePath, Map<String, String> param)
			throws Exception {
		// 待请求参数数组
		Map<String, String> finalParam = buildRequestPara(param);
		HttpPost httpPost = new HttpPost(ALIPAY_GATEWAY_NEW + "_input_charset=" + AlipayConfig.input_charset);
		RequestConfig requestConfig = RequestConfig.custom().setSocketTimeout(TIME_OUT).setConnectTimeout(TIME_OUT)
				.build();// 设置请求和传输超时时间
		httpPost.setConfig(requestConfig);
		if (null != finalParam && !finalParam.isEmpty()) {
			List<NameValuePair> parameters = new ArrayList<NameValuePair>();
			for (String key : finalParam.keySet()) {
				parameters.add(new BasicNameValuePair(key, finalParam.get(key)));
			}
