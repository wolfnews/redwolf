...
public class SQLUtils {
	...
	private static Map<String, List<FieldInfo>> cacheMap = new HashMap<String, List<FieldInfo>>();
	...
	public static List<FieldInfo> loadPojoSqlInfo(Class<?> pojoClass) {
		...
		if (null == cacheMap.get(pojoClass.getName())) {
			...
			for (Field field : fields) {
				...
				if (field.isAnnotationPresent(com.hoteam.wolf.jdbc.annotations.Field.class)) {
					String value = (field.getAnnotation(com.hoteam.wolf.jdbc.annotations.Field.class)).value();// 得到配置的数据库字段名
	...
	// String sql="";
	// try {
	// sql = SQLUtils.buildDeleteSqlByColumns(Channel.class, columns);
	// } catch (Exception e) {
