...
jQuery(function($) {
	...
	jQuery(grid_selector).jqGrid({
		...
		height : '100%',
		autowidth: true,
		colNames : ['标题', '关键字', '状态','点赞数量','生成日期','快捷操作'],
		colModel : [ 
			...
			{name : 'gmtCreate',index : 'gmtCreate',width : 15},
			{
				...
				formatter : function(cellvalue, options,rowObject) {
					...
					return "<button class=\"btn btn-xs btn-danger\" onclick=\"showNotice('"+rowObject.id+ "','"+rowObject.status+"')\"><b>"+label+"</b></button> &nbsp;"
							+"<button class=\"btn btn-xs btn-danger\" onclick=\"removeNotice('"+rowObject.id+ "')\"><b>删除</b></button> &nbsp;";
				}
		    }
		],
		viewrecords : true,
